...
The `Form` component under the hood is powered by [Formik](https://formik.org/docs/overview). Formik manages form state, validation, and form submission.
...
The following example creats a basic text input field using the Formik `<Field />` component. The `name` prop on the `field` is set to `firstName` which is used for tracking the import state.
